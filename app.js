...
function loadLevel(levelIndex){
  ...
  level.surpriseBlock.forEach((blockData,index)=>{
...
// Enemy movement and collision
  for (let enemy of gameObjects.enemies) {
    ...
    // Platform detection and edge turning
    let onPlatform = false
    for (let platform of gameObjects.platforms) {
      if (enemy.x + enemy.width > platform.x &&
        enemy.x < platform.x + platform.width &&
        enemy.y + enemy.height >= platform.y-5 &&
        enemy.y + enemy.height <= platform.y +5) // Added small threshold
      {
        onPlatform = true
        break
      }
...
// Surprise blocks
  for (let block of gameObjects.surpriseBlocks) {
    if (!block.hit && checkCollision(player, block) && player.velocityY < 0) {
      ...
      spawItemonBox(block,block.type)
...
if(player.bigTimer) {
  ...
  function spawItemonBox(block,type){
  const gameArea=document.getElementById('game-area')
